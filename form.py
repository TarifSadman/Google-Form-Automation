...
def parse_form_entries(url: str, only_required=False):
    ...
    def parse_entry(entry):
        ...
        if entry[4] is None:
            print(f"Warning: Entry {entry_name} has no sub_entries.")
            return []
        ...
        for sub_entry in entry[4]:
            info = {
                ...
                "name":
                ' - '.join(sub_entry[3]) if
                (len(sub_entry) > 3 and sub_entry[3]) else None,
                "options": [(x[0] or ANY_TEXT_FIELD)
                            for x in sub_entry[1]] if sub_entry[1] else None,
            }
    ...
    if page_count > 0:
        parsed_entries.append({
            ...
            False,
            "options":
            "from 0 to (number of page - 1)",
            "default_value":
            ','.join(map(str, range(page_count + 1)))
        })
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Google Form Autofill and Submit")
    ...
    get_form_submit_request(args.url, args.output, args.required,
                            not args.no_comment)
